...
def test_response_store_configuration(mode, enabled, cli):
    if cli:
        ...
        args = ServerArgs(
            model_path="dummy", disaggregation_mode=mode, enable_response_store=enabled
        )
    if enabled and mode != "null":
        ...
        assert serving.is_disaggregated is (mode != "null")


@pytest.mark.parametrize(
    "harmony,stream,mode,enabled,store",
    [
        (False, False, "null", False, "omitted"),
        (False, True, "null", False, "omitted"),
        (True, False, "null", False, "omitted"),
        (True, True, "null", False, "omitted"),
        (False, False, "prefill", False, "omitted"),
        (False, True, "decode", False, "omitted"),
        (False, False, "null", True, None),
        (False, True, "null", True, None),
    ],
)
def test_response_store_output_and_history(
    response_serving, harmony, stream, mode, enabled, store
):
    serving = response_serving(enabled=enabled, mode=mode, harmony=harmony)
    kwargs = {} if store == "omitted" else {"store": store}
    request = ResponsesRequest(model="x", input="hi", stream=stream, **kwargs)
    response = asyncio.run(create_response_result(serving, request))
    assert response.status == "completed"
    assert response.output[0].content[0].text == "ok"
    assert response.store == request.store
    assert not serving.background_tasks
    assert not serving.response_store
    assert not serving.msg_store


@pytest.mark.parametrize(
    "mode,stream,previous",
    [
        ("null", False, "resp_missing"),
        ("prefill", False, "invalid"),
        ("decode", True, ""),
    ],
)
def test_disabled_previous_admission(response_serving, mode, stream, previous):
    serving = response_serving(mode=mode)
    ...
    assert_response_error(
        asyncio.run(serving.create_responses(request)), "previous_response_id"
    )
...
@pytest.mark.parametrize(
    "mode,enabled,stream,store",
    [
        ("null", False, False, True),
        ("prefill", False, True, False),
        ("decode", False, False, None),
        ("null", True, False, False),
        ("null", True, True, False),
        ("null", True, True, None),
    ],
)
def test_background_store_admission(response_serving, mode, enabled, stream, store):
    serving = response_serving(enabled=enabled, mode=mode)
    serving._make_request = AsyncMock(side_effect=AssertionError("prompt reached"))
    request = ResponsesRequest(
        model="x", input="hi", background=True, stream=stream, store=store
    )
    result = asyncio.run(serving.create_responses(request))
    assert_response_error(
        result,
        "store" if enabled else "background",
        "background=true requires store=true." if enabled else STORE_DISABLED_MESSAGE,
    )
    serving._make_request.assert_not_called()
    serving.tokenizer_manager.generate_request.assert_not_called()
    assert (
        not serving.msg_store
        and not serving.response_store
        and not serving.background_tasks
    )


@pytest.mark.parametrize(
    "mode,response_id",
    [("null", "bad"), ("prefill", "resp_missing"), ("decode", "bad")],
)
def test_disabled_read_endpoints(response_serving, mode, response_id):
    serving = response_serving(mode=mode)
    serving.response_store = Mock()
    serving.background_tasks = Mock()
    for method in (serving.retrieve_responses, serving.cancel_responses):
        assert_response_error(asyncio.run(method(response_id)), "response_id")
    serving.tokenizer_manager.abort_request.assert_not_called()
    assert not serving.response_store.mock_calls
    assert not serving.background_tasks.mock_calls


def test_enabled_read_endpoints_and_predecessor_validation(response_serving):
...
def test_active_stream_cancel_and_final_history(response_serving, harmony):
    serving = response_serving(enabled=True, harmony=harmony)
...
def test_non_storing_continuation_replays_stored_output(
    response_serving, harmony, stream
):
    serving = response_serving(enabled=True, harmony=harmony)
...
@pytest.mark.parametrize(
    "harmony,stream", [(False, False), (False, True), (True, True)]
)
@pytest.mark.parametrize("enabled", [False, True])
def test_completion_persistence_guard(response_serving, stream, enabled, harmony):
    serving = response_serving(enabled=enabled, harmony=harmony)
    ...
    if enabled:
        ...
        serving.response_store = Mock()
        serving.msg_store = Mock()
    if stream and not harmony:
        StreamFixture(serving, request).run([engine_chunk("ok", finish=True)])
    else:
        if harmony:
            messages = serving._construct_input_messages_with_harmony(request, None)
            context_type = StreamingHarmonyContext if stream else HarmonyContext
    ...
    if enabled:
        ...
        assert (
            not serving.response_store.mock_calls and not serving.msg_store.mock_calls
        )
...
@pytest.mark.parametrize(
    ...
    [
        ("prefill", "web_search"),
        ("decode", "web_search_preview"),
        ("decode", "code_interpreter"),
    ],
)
def test_pd_builtin_tool_admission(response_serving, mode, tool):
    serving = response_serving(mode=mode, harmony=True)
    serving.tool_server = Mock()
    serving.supports_browsing = True
    serving.supports_code_interpreter = True
    request = ResponsesRequest(model="x", input="hi", tools=[{"type": tool}])
    result = asyncio.run(serving.create_responses(request))
